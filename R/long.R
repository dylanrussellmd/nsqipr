make_long <- function(df) {
  ...
  df <- df[, "retor3podays" := NA_integer_]
  df <- df[, c("reopor3cpt1", "reopor3icd91", "reopor3icd101") := NA_character_]
  df <- df[, "retor3related" := NA]
  ...
  na.omit(data.table::melt(df, id.vars = "caseid", measure.vars = list(reoperations, retorpodays, reoporcpt1, retorrelated, reoporicd91, reoporicd10),
