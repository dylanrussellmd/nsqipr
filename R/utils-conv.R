conv_to_standard <- function(file, set, write_to_csv, append, return_df) {
...
set_up_df <- function(df, col_names) { # col_names needs to be dynamically selected from a list of col_name vectors depending on file type
  df %>%
...
reorder_cols <- function(df) {
  df %>%
    dplyr::select(dplyr::any_of(caseid, pufyear, sex,  race, ethnicity_hispanic, prncptx, cpt, workrvu, inout, transt, age, admyr,
