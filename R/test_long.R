...
#' @param reorder logical vector indicating whether records should be renumbered after removal
...
make_commas_long <- function(df, cols, variable.name, levels) {
  value.name <- deparse(substitute(cols))

  if(check_if_in(value.name, df)) {

    mat <- stringi::stri_split_regex(df[[value.name]], ",\\s?", simplify = NA, n = length(cols), omit_empty = TRUE, opts_regex = list(case_insensitive = TRUE))
    for(j in seq_along(cols)) data.table::set(df, j = cols[[j]], value = mat[, j])
    ...
    if(!rlang::is_missing(levels)) data.table::set(melted, j  = value.name, value = melted[[value.name]] %^% levels)
