...
## Use
  ...
  1. Excute all `.exe` executable files from ACS NSQIP<sup>&copy;</sup> in a single directory (`dir`) (you may, but it is not recommended to, change the default file names). 
  2. Now simply run `nsqip(dir)`.
  
`nsqipr` will take care of the data importing and data cleaning. You're now ready to use the ACS NSQIP<sup>&copy;</sup> for data analysis!
...
`nsqipr` significant reduces the file sizes after cleaning the data.
