...
A local or remote database to which you have read/write privileges needs to be available. At the time, only a PostgreSQL server is compatible (recommend PostgreSQL 12). If you currently do not have one available, one can be easily and cheaply procured with [Google Cloud](https://cloud.google.com/). 
...
```{r}
readr::read_csv("../nsqipr-misc/allvars.csv", # This function will show me which columns have non-matching rows.
                      na = "",
  ...
  dplyr::mutate(dplyr::across(2:dplyr::last_col(), ~dplyr::if_else(.x==TRUE, "&#9989;","&#10060;"))) %>% 
  knitr::kable(caption =  "ACS NSQIP<sup>&copy;</sup> Variables", align = "c")
```
